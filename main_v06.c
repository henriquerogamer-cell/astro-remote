...
static int already_has_path(astro_elf_item_t *items, int count, const char *path)
{
  int i;
  for (i = 0; i < count; i++) {
    if (!strcmp(items[i].path, path)) return 1;
  }
  return 0;
}
...
static void send_api_status_v06(int fd)
{
  ...
  snprintf(json, sizeof(json),
    "{\"ok\":true,\"console\":\"PlayStation 5\",\"console_online\":true,\"astro_online\":true,\"http_port\":%d,\"version\":\"0.6-curated-library\",\"uptime_seconds\":%ld,\"remote\":\"standby\",\"elf_count\":%d,\"elf_count_source\":\"payload-roots\"}",
    ASTRO_PORT, uptime, elf_count);
  send_json(fd, "200 OK", json);
}

static void send_api_elfs_v06(int fd)
{
  astro_elf_item_t items[ASTRO_MAX_ELFS];
...
static void send_dashboard_v06(int fd)
{
...
int main(void)
{
  ...
  notify("ASTRO Remote v0.6 curated library - porta 45821");
  ...
  while (running) {
    ...
    else if (strstr(buffer, "GET /api/status ") && logged(buffer)) send_api_status_v06(client_fd);
    else if (strstr(buffer, "GET /api/elfs ") && logged(buffer)) send_api_elfs_v06(client_fd);
    ...
    else if (strstr(buffer, "GET / ") && logged(buffer)) send_dashboard_v06(client_fd);
