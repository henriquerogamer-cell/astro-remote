...
static char hidden[ASTRO_MAX_HIDDEN][128];
...
{
  size_t j=0;
  while(*src&&j+1<n){unsigned char c=(unsigned char)*src++;if(isalnum(c))dst[j++]=(char)tolower(c);else if((c=='-'||c=='_'||c=='.')&&j)dst[j++]='-';}
...
{
  astro_service_t *s=by_process(process);
  if(s){if(port>0)s->port=port;return;}
  if(service_count>=ASTRO_MAX_SERVICES)return;
  s=&services[service_count++]; memset(s,0,sizeof(*s));
  snprintf(s->id,sizeof(s->id),"%s",id); snprintf(s->name,sizeof(s->name),"%s",name); snprintf(s->process,sizeof(s->process),"%s",process); s->port=port;
}
...
{
  FILE *f; char line[512]; service_count=0;
...
static int is_hidden(const char *p){int i;for(i=0;i<hidden_count;i++)if(!strcasecmp(hidden[i],p))return 1;return 0;}

static void save_hidden(void)
{
  FILE *f; int i; mkdir(ASTRO_REGISTRY_DIR,0755); f=fopen(ASTRO_HIDDEN_FILE,"w"); if(!f)return;
  for(i=0;i<hidden_count;i++)fprintf(f,"%s\n",hidden[i]); fclose(f);
}

static void load_hidden(void)
{
  FILE *f; char line[128]; hidden_count=0; f=fopen(ASTRO_HIDDEN_FILE,"r"); if(!f)return;
  while(hidden_count<ASTRO_MAX_HIDDEN&&fgets(line,sizeof(line),f)){
    line[strcspn(line,"\r\n")]='\0'; if(!line[0]||is_hidden(line))continue;
    snprintf(hidden[hidden_count++],sizeof(hidden[0]),"%s",line);
  }
  fclose(f);
}

static void hide_process(const char *p)
{
  if(!p[0]||is_hidden(p)||hidden_count>=ASTRO_MAX_HIDDEN)return;
  snprintf(hidden[hidden_count++],sizeof(hidden[0]),"%s",p); save_hidden();
}

static void unhide_process(const char *p)
{
  int i; for(i=0;i<hidden_count;i++)if(!strcasecmp(hidden[i],p)){int j;for(j=i;j+1<hidden_count;j++)snprintf(hidden[j],sizeof(hidden[j]),"%s",hidden[j+1]);hidden_count--;save_hidden();return;}
}

static int extract_form(const char *request,const char *key,char *out,size_t n)
{
  const char *b=strstr(request,"\r\n\r\n"),*p; char enc[1024],needle[64]; size_t i=0;
  if(!b||!n)return 0; b+=4; snprintf(needle,sizeof(needle),"%s=",key); p=strstr(b,needle); if(!p)return 0; p+=strlen(needle);
...
static int extract_processes(const char *http,char names[][128],int max)
{
  ...
  while((p=strstr(p,".elf"))!=NULL&&count<max){const char *s=p,*e=p+4;size_t len;int dup=0,i;
...
{
  char pr[ASTRO_HTTP_BUF],json[24576],names[64][128];size_t used=0;int visible=0,i,pc=0,ok=http_get_local(PROSPERO_PORT,"/api/processes",pr,sizeof(pr))==0;
  ...
  if(ok){pc=extract_processes(pr,names,64);for(i=0;i<pc;i++){astro_service_t *s=by_process(names[i]);if(is_hidden(names[i]))continue;if(s&&s->port>0&&tcp_open(s->port)){used+=(size_t)snprintf(json+used,sizeof(json)-used,"%s{\"id\":\"%s\",\"name\":\"%s\",\"process\":\"%s\",\"port\":%d,\"known\":true,\"active\":true,\"url\":\"/service/%s/\"}",visible?",":"",s->id,s->name,s->process,s->port,s->id);visible++;}else if(!s){char id[48];slugify(names[i],id,sizeof(id));used+=(size_t)snprintf(json+used,sizeof(json)-used,"%s{\"id\":\"%s\",\"name\":\"Novo serviço detectado\",\"process\":\"%s\",\"port\":0,\"known\":false,\"active\":true}",visible?",":"",id,names[i]);visible++;}}}
  used+=(size_t)snprintf(json+used,sizeof(json)-used,"],\"count\":%d,\"hidden\":[",visible);
  for(i=0;i<hidden_count;i++)used+=(size_t)snprintf(json+used,sizeof(json)-used,"%s{\"process\":\"%s\"}",i?",":"",hidden[i]);
  snprintf(json+used,sizeof(json)-used,"],\"hidden_count\":%d}",hidden_count);send_json(fd,"200 OK",json);
}
...
static void send_register(int fd,const char *req)
{
  ...
  slugify(process,id,sizeof(id));add_service(id,name,process,port);unhide_process(process);save_registry();send_json(fd,"200 OK","{\"ok\":true,\"saved\":true}");
}

static void send_hide(int fd,const char *req,int unhide)
{
  char process[128];if(!extract_form(req,"process",process,sizeof(process))){send_json(fd,"400 Bad Request","{\"ok\":false,\"error\":\"missing_process\"}");return;}
  if(unhide)unhide_process(process);else hide_process(process);send_json(fd,"200 OK","{\"ok\":true}");
}

static void send_status(int fd)
{
  char j[512];time_t now=time(NULL);long up=now>=astro_started_at?(long)(now-astro_started_at):0;
  snprintf(j,sizeof(j),"{\"ok\":true,\"console\":\"PlayStation 5\",\"console_online\":true,\"astro_online\":true,\"http_port\":%d,\"version\":\"0.12-hidden-services\",\"uptime_seconds\":%ld,\"remote\":\"standby\",\"prospero_online\":%s}",ASTRO_PORT,up,tcp_open(PROSPERO_PORT)?"true":"false");send_json(fd,"200 OK",j);
}

static const char *dash_script=
"<style>.service-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;padding:14px}.service-card{background:#11161d;border:1px solid #29313d;border-radius:9px;padding:14px}.service-top{display:flex;justify-content:space-between;gap:10px}.service-card h4{margin:0;font-size:14px}.service-process{color:#687487;font-size:10px;margin:5px 0 12px}.service-state{font-size:10px;padding:4px 7px;border-radius:999px;font-weight:800;background:#10251b;border:1px solid #24553a;color:#7fe4aa}.service-chip{font-size:10px;padding:4px 7px;border-radius:5px;background:#192431;border:1px solid #293c50;color:#91bce9}.service-open,.service-save,.service-hide,.service-restore{margin-top:10px;height:34px;border-radius:7px;border:1px solid #176fdc;background:linear-gradient(#2084ff,#1269dc);color:#fff;font-weight:750;width:100%;cursor:pointer}.service-hide{background:#171d25;border-color:#3a4554;color:#aeb8c6}.service-restore{background:#17251d;border-color:#315b3d;color:#9de4b1}.service-input{width:100%;margin-top:8px;height:34px;border-radius:7px;border:1px solid #303947;background:#0d1218;color:#fff;padding:0 10px}.service-tabs{display:flex;gap:8px;padding:0 14px 4px}.service-tab{border:1px solid #303947;background:#121821;color:#8f9aaa;border-radius:7px;padding:7px 11px;cursor:pointer;font-size:11px;font-weight:750}.service-tab.active{background:#18283a;border-color:#236fc5;color:#b8d9ff}@media(max-width:900px){.service-grid{grid-template-columns:1fr}}</style>"
"<script>"
"let svcTab='active',svcData=null;"
"function esc(s){return String(s).replace(/[&<>\"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',\"'\":'&#39;'}[m]))}"
"async function act(url,p){const r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'process='+encodeURIComponent(p)});if(r.ok)refresh(true)}"
"function card(x){const c=document.createElement('div');c.className='service-card';if(x.known){c.innerHTML=\"<div class='service-top'><h4>\"+esc(x.name)+\"</h4><span class='service-state'>ATIVO</span></div><div class='service-process'>\"+esc(x.process)+\"</div><span class='service-chip'>INTERNO · 127.0.0.1:\"+x.port+\"</span><button class='service-open'>Abrir dentro do Astro</button>\";c.querySelector('.service-open').onclick=()=>location.href=x.url}else{c.innerHTML=\"<div class='service-top'><h4>Novo serviço detectado</h4><span class='service-state'>NOVO</span></div><div class='service-process'>\"+esc(x.process)+\"</div><input class='service-input nm' placeholder='Nome do serviço'><input class='service-input pt' inputmode='numeric' placeholder='Porta web'><button class='service-save'>Salvar e ativar proxy</button><button class='service-hide'>Ocultar</button>\";c.querySelector('.service-save').onclick=async()=>{const nm=c.querySelector('.nm').value||x.process,pt=c.querySelector('.pt').value;const r=await fetch('/api/services/register',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'process='+encodeURIComponent(x.process)+'&name='+encodeURIComponent(nm)+'&port='+encodeURIComponent(pt)});if(r.ok)refresh(true)};c.querySelector('.service-hide').onclick=()=>act('/api/services/hide',x.process)}return c}"
"function hiddenCard(x){const c=document.createElement('div');c.className='service-card';c.innerHTML=\"<div class='service-top'><h4>Oculto</h4><span class='service-state' style='background:#1a2029;border-color:#303947;color:#9ba7b6'>OCULTO</span></div><div class='service-process'>\"+esc(x.process)+\"</div><button class='service-restore'>Restaurar para serviços</button>\";c.querySelector('.service-restore').onclick=()=>act('/api/services/unhide',x.process);return c}"
"function render(){if(!svcData)return;const sec=document.querySelector('.two .section');if(!sec)return;let tabs=sec.querySelector('.service-tabs');if(!tabs){tabs=document.createElement('div');tabs.className='service-tabs';sec.insertBefore(tabs,sec.querySelector('.service-grid'))}tabs.innerHTML=\"<button class='service-tab \"+(svcTab==='active'?'active':'')+\"'>Serviços</button><button class='service-tab \"+(svcTab==='hidden'?'active':'')+\"'>Ocultos (\"+svcData.hidden_count+\")</button>\";tabs.children[0].onclick=()=>{svcTab='active';render()};tabs.children[1].onclick=()=>{svcTab='hidden';render()};let g=sec.querySelector('.service-grid');g.innerHTML='';if(svcTab==='active'){svcData.items.forEach(x=>g.appendChild(card(x)));if(!svcData.items.length)g.innerHTML=\"<div style='color:#8f9aaa'>Nenhum serviço ativo detectado.</div>\"}else{svcData.hidden.forEach(x=>g.appendChild(hiddenCard(x)));if(!svcData.hidden.length)g.innerHTML=\"<div style='color:#8f9aaa'>Nenhum serviço oculto.</div>\"}}"
"async function refresh(force){if(!force&&document.activeElement&&document.activeElement.classList.contains('service-input'))return;try{const [sr,vr]=await Promise.all([fetch('/api/status',{cache:'no-store'}),fetch('/api/services',{cache:'no-store'})]);const s=await sr.json();svcData=await vr.json();const top=document.querySelector('.status');if(top)top.innerHTML=\"<span class='dot'></span>PS5 ONLINE\";const m=document.querySelectorAll('.metric');if(m.length>=4){m[1].querySelector('strong').textContent=String(s.http_port);m[1].querySelector('small').textContent='API '+s.version+' ativa';m[3].querySelector('strong').textContent=String(svcData.count);m[3].querySelector('small').textContent='serviços/processos detectados'}const sec=document.querySelector('.two .section');if(sec){const head=sec.querySelector('.section-head');if(head){head.querySelector('h3').textContent='Serviços ativos';head.querySelector('span').textContent=svcData.manager_online?'via Próspero':'Próspero offline'}let g=sec.querySelector('.service-grid');if(!g){const old=sec.querySelector('.table');if(old)old.remove();g=document.createElement('div');g.className='service-grid';sec.appendChild(g)}render()}}catch(e){const top=document.querySelector('.status');if(top)top.textContent='ASTRO API OFFLINE'}}"
"document.addEventListener('DOMContentLoaded',()=>{refresh(true);setInterval(()=>refresh(false),8000)});"
"</script>";
...
int main(void)
{
  int server,client,opt=1;struct sockaddr_in a;char buf[8192];running=1;astro_started_at=time(NULL);load_registry();load_hidden();
  server=socket(AF_INET,SOCK_STREAM,0);if(server<0)return 1;setsockopt(server,SOL_SOCKET,SO_REUSEADDR,&opt,sizeof(opt));memset(&a,0,sizeof(a));a.sin_family=AF_INET;a.sin_addr.s_addr=INADDR_ANY;a.sin_port=htons(ASTRO_PORT);if(bind(server,(struct sockaddr*)&a,sizeof(a))<0){close(server);return 1;}if(listen(server,8)<0){close(server);return 1;}notify("ASTRO Remote v0.12 hidden services - porta 45821");
