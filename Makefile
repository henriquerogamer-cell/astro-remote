...
main_v137_build.c: main_v136.c main_v132_embed.c main_v13_embed.c main_v12_embed.c
	sed 's/astro_remote_service_init();/astro_remote_service_init(); astro_remote_service_ensure_worker();/' main_v136.c > $@

$(ELF): $(CHIAKI_RPCRYPT) main_v137_build.c remote_service.c remote_service.h remote_worker.c remote_worker.h remote_ps5_source.c remote_ps5_source.h remote_pairing.c remote_pairing.h remote_page_v137.h astro_pair_client.c astro_pair_client.h astro_pair_crypto.c astro_pair_crypto.h chiaki_mbedtls_compat.c main_v132_embed.c main_v13_embed.c main_v12_embed.c main_v03.c astro_process_name.c
	$(CC) $(CFLAGS) $(PAIR_CFLAGS) -o $@ main_v137_build.c remote_service.c remote_worker.c remote_ps5_source.c remote_pairing.c astro_pair_client.c astro_pair_crypto.c chiaki_mbedtls_compat.c $(CHIAKI_RPCRYPT) astro_process_name.c $(LDADD)
