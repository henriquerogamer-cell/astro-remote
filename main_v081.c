...
static void astro_service_meta(const char *name, int *is_service, int *port, int *active, int *stoppable)
{
  *is_service = 0; *port = 0; *active = 0; *stoppable = 0;
  ...
  if (!strcmp(name, "websrv-ps5.elf") || !strcmp(name, "websrv.elf")) {
    *is_service = 1; *port = 8080; *active = astro_port_open(*port); return;
  }
...
static void send_api_status_v081(int fd)
{
  astro_elf_item_t items[ASTRO_MAX_ELFS];
  ...
  snprintf(json, sizeof(json),
    "{\"ok\":true,\"console\":\"PlayStation 5\",\"console_online\":true,\"astro_online\":true,\"http_port\":%d,\"version\":\"0.8.1-websrv-detect\",\"uptime_seconds\":%ld,\"remote\":\"standby\",\"elf_count\":%d,\"elfldr_port\":%d}",
    ASTRO_PORT, uptime, elf_count, ASTRO_ELFLDR_PORT);
  send_json(fd, "200 OK", json);
}

static void send_api_elfs_v081(int fd)
{
  astro_elf_item_t items[ASTRO_MAX_ELFS];
  int count = astro_collect_elfs(items, ASTRO_MAX_ELFS);
  char json[32768];
...
static void send_api_launch_v081(int fd, const char *request)
{
  char path[ASTRO_PATH_MAX];
...
static void send_api_stop_v081(int fd, const char *request)
{
  char path[ASTRO_PATH_MAX];
  astro_elf_item_t item;
  int rc;
...
static const char *dashboard_v081_script =
"<style>"
...
static void send_dashboard_v081(int fd)
{
...
int main(void)
{
  ...
  notify("ASTRO Remote v0.8.1 websrv detect - porta 45821");
  ...
  while (running) {
    ...
    else if (strstr(buffer, "GET /api/status ") && logged(buffer)) send_api_status_v081(client_fd);
    else if (strstr(buffer, "GET /api/elfs ") && logged(buffer)) send_api_elfs_v081(client_fd);
    else if (strstr(buffer, "POST /api/elfs/launch ") && logged(buffer)) send_api_launch_v081(client_fd, buffer);
    else if (strstr(buffer, "POST /api/services/stop ") && logged(buffer)) send_api_stop_v081(client_fd, buffer);
    ...
    else if (strstr(buffer, "GET / ") && logged(buffer)) send_dashboard_v081(client_fd);
