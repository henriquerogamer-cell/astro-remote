...
{
  static const char *roots[] = { "/data", "/mnt/sandbox" };
  int count = 0;
  size_t i;
...
static void send_api_status_v051(int fd)
{
  ...
  snprintf(
    json,
    sizeof(json),
    "{\"ok\":true,\"console\":\"PlayStation 5\",\"console_online\":true,\"astro_online\":true,\"http_port\":%d,\"version\":\"0.5.1-elf-discovery\",\"uptime_seconds\":%ld,\"remote\":\"standby\",\"elf_count\":%d,\"elf_count_source\":\"filesystem\"}",
    ASTRO_PORT,
    uptime,
    elf_count
  );
...
static void send_api_elfs(int fd)
{
  ...
  used += (size_t)snprintf(json + used, sizeof(json) - used, "{\"ok\":true,\"count\":%d,\"roots\":[\"/data\",\"/mnt/sandbox\"],\"items\":[", count);
  ...
  snprintf(json + used, sizeof(json) - used, "],\"truncated\":%s}", count >= ASTRO_MAX_ELFS ? "true" : "false");
  send_json(fd, "200 OK", json);
}

static const char *dashboard_v051_script =
"<script>"
"function fmtBytes(n){if(n<1024)return n+' B';if(n<1048576)return (n/1024).toFixed(1)+' KB';return (n/1048576).toFixed(1)+' MB'}"
"async function astroRefresh(){try{"
...
static void send_dashboard_v051(int fd)
{
  ...
  script_len = strlen(dashboard_v051_script);
  ...
  send(fd, dashboard_v051_script, script_len, 0);
...
int main(void)
{
  ...
  notify("ASTRO Remote v0.5.1 ELF discovery - porta 45821");
  ...
  while (running) {
    ...
    if (strstr(buffer, "POST /login ") && valid_login(buffer)) {
      ...
      send_api_status_v051(client_fd);
